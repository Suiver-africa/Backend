...
const VerifyPin = () => {
  const { originalPin } = useLocalSearchParams<{ originalPin: string }>();
    ...
    if (pin.length < 4) {
      ...
      if (newPin.length === 4) {
        if (newPin === originalPin) {
          // Persist auth state — use a placeholder token; replace with real API token if available
          await signIn("auth_token_placeholder", newPin);
          setTimeout(() => {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.topSection}>
          ...
          <Text style={[styles.subtitle, error && styles.subtitleError]}>
            {error ? "Pins don't match, please try again" : "confirm you entered the right pin"}
          </Text>
