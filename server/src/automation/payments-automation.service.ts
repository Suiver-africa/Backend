...
export class PaymentsAutomationService implements OnModuleInit {
  ...
  constructor(
    ...
    const hasMoralis = !!(
      process.env.MORALIS_API_KEY || this.config.get('MORALIS_API_KEY')
    );
    const hasHelius = !!(
      process.env.HELIUS_API_KEY || this.config.get('HELIUS_API_KEY')
    );
    const hasBlock = !!(
      process.env.BLOCKSTREAM_API || this.config.get('BLOCKSTREAM_API')
    );
    ...
    if (process.env.PAYSTACK_SECRET || this.config.get('PAYSTACK_SECRET'))
      this.paystack = paystackProvider;
    if (process.env.FLUTTERWAVE_SECRET || this.config.get('FLUTTERWAVE_SECRET'))
      this.flutterwave = flutterProvider;
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      'PaymentsAutomationService initialized with watcher: ' +
        this.watcher.constructor.name,
    );
  }
  ...
  async handleDeposit(evt: DepositEvent) {
    ...
    if (!userId) {
      this.logger.warn(
        `Deposit received for unknown address ${evt.address}; ignoring.`,
      );
      return;
    }
    ...
    if (evt.confirmations < required) {
      this.logger.log(
        `Tx ${evt.txHash} has ${evt.confirmations} confirmations; waiting for ${required}`,
      );
    ...
    const humanAmount =
      Number(evt.amount) /
      (evt.symbol === 'BTC' ? 1e8 : evt.symbol === 'SOL' ? 1e9 : 1e6);
    ...
    try {
      if (this.paystack) {
        ...
        if (!recipient)
          throw new Error(
            'PAYSTACK_RECIPIENT_CODE not configured; cannot transfer',
          );
        const resp = await this.paystack.initiateTransfer({
          recipient,
          amount: payoutNgn * 100,
          reference: 'swap_' + tx.id,
        });
        await this.prisma.transaction.update({
          where: { id: tx.id },
          data: { status: 'PROCESSING', metadata: { payout: resp } },
        });
        ...
        const resp = await this.flutterwave.transfer({
          account_bank: bank,
          account_number: acct,
          amount: payoutNgn,
        ...
        this.logger.log(
          'Flutterwave transfer response: ' + JSON.stringify(resp),
        );
      } else {
        this.logger.log(
          'No payout provider configured; marking transaction for manual processing',
        );
        await this.prisma.transaction.update({
          where: { id: tx.id },
          data: {
            status: 'PROCESSING',
            metadata: { note: 'Awaiting payout provider' },
          },
        });
      }
    } catch (err) {
      ...
      await this.prisma.transaction.update({
        where: { id: tx.id },
        data: { status: 'FAILED', metadata: { error: '' + err } },
      });
    }
  }
}
