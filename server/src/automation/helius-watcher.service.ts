...
export class HeliusWatcherService implements DepositWatcher {
  ...
  constructor(private config: ConfigService) {
    this.apiKey = this.config.get<string>('HELIUS_API_KEY') || process.env.HELIUS_API_KEY || '';
    if (!this.apiKey) this.logger.warn('Helius API key not configured; HeliusWatcher disabled');
  }
  ...
  async stop() { if (this.interval) clearInterval(this.interval); this.logger.log('Stopped Helius watcher'); }
    ...
    try {
      ...
      for (const address of watched) {
        ...
        for (const tx of items) {
          ...
          const evt: DepositEvent = { address, txHash: String(txHash), amount: '0', symbol: 'SOL', confirmations: 1, chain: 'SOL' };
          await this.cb(evt);
        }
      }
    } catch (err) {
