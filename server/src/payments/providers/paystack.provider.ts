...
export class PaystackProvider {
  ...
  async createRecipient({ name, account_number, bank_code }: { name: string; account_number: string; bank_code: string }) {
    if (!this.secret) throw new Error('Paystack secret not configured');
    const url = `${this.base}/transferrecipient`;
    const payload = { type: 'nuban', name, account_number, bank_code, currency: 'NGN' };
    const res = await axios.post(url, payload, { headers: this.headers() });
    return res.data;
  }

  async initiateTransfer({ recipient, amount, reference }: { recipient: string; amount: number; reference?: string }) {
    if (!this.secret) throw new Error('Paystack secret not configured');
