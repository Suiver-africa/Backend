...
export class EvmListener {
    ...
    setInterval(async () => {
      try {
        ...
        if (currentBlock > lastProcessedBlock) {
          for (
            let blockNum = lastProcessedBlock + BigInt(1);
    ...
    for (const tx of transactions) {
      if (tx.to && watchedAddresses.has(tx.to.toLowerCase())) {
        const wallet = wallets.find(
          (w) => w.address && w.address.toLowerCase() === tx.to.toLowerCase(),
        );

        if (!wallet) {
          this.logger.warn(
            `Wallet not found for address ${tx.to} despite being in watched list`,
          );
          continue;
        }
        ...
        this.logger.log(
          `Detected incoming transaction: ${tx.hash} for ${tx.to}`,
        );
  ...
  async simulateDeposit(
    ...
    try {
      ...
      this.logger.error(
        `Failed to process simulated deposit: ${e.message}`,
        e.stack,
      );
    }
  }
