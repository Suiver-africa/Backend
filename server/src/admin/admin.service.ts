...
export class AdminService {
  ...
  async getChartData(days: number) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      // Day label
      const dayLabel: string = startOfDay.toLocaleDateString('en-US', {
        weekday: 'short',
      });
  ...
  async getUsers(search?: string) {
    ...
    return users.map((user) => ({
      ...
      walletBalance: user.wallets.reduce(
        (sum, w) => sum + Number(w.nairaBalance),
        0,
      ),
  ...
  async approvePayout(id: string) {
    ...
    return {
      success: true,
      message: 'Payout approved and triggered via Paystack',
    };
  }
