...
export class CryptoService {
  ...
  private generateSuiWallet() {
    ...
    const address =
      '0x' + privateKey.slice(0, 20).toString('hex').padStart(40, '0');
  ...
  private async getSuiBalance(address: string): Promise<number> {
    try {
      ...
      const response = await fetch(
        this.configService.get('SUI_RPC_URL') ||
      ...
      const data = await response.json();
      return data.result
        ? parseInt(data.result.totalBalance) / Math.pow(10, 9)
        : 0;
    } catch (error) {
  ...
  private async getAptosBalance(address: string): Promise<number> {
    try {
      ...
      const response = await fetch(
        `${this.configService.get('APTOS_RPC_URL') || 'https://fullnode.mainnet.aptoslabs.com/v1'}/accounts/${address}/resource/0x1::coin::CoinStore%3C0x1::aptos_coin::AptosCoin%3E`,
      );
      const data = await response.json();
  ...
  async convertCryptoToNaira(
    amount: number,
    cryptocurrency: string,
  ): Promise<{ nairaAmount: number; exchangeRate: number }> {
    const prices = await this.getCryptoPrices();
    const cryptoPrice = prices.find(
      (p) => p.symbol.toLowerCase() === cryptocurrency.toLowerCase(),
    );
